frappe.ui.form.on("AI Provider Settings", {
  refresh(frm) {
    frm.add_custom_button("Test AI Provider", async () => {
      ...
      dialog.show();
      dialog.get_field("result").$wrapper.html("<p>Testing AI provider connection...</p>");

      try {
        const response = await frappe.call({
          method: "erp_ai_assistant.api.assistant.test_ai_provider_connection",
        });
        const result = response.message || {};
        const indicator = result.ok ? "green" : "red";
        const preview = result.preview
          ? `<p><strong>Preview:</strong> ${frappe.utils.escape_html(result.preview)}</p>`
          : "";
        const detail = result.detail
          ? `<p><strong>Detail:</strong> ${frappe.utils.escape_html(result.detail)}</p>`
          : "";
        ...
        dialog.get_field("result").$wrapper.html(`
          <div>
            ...
            <p><strong>Provider:</strong> ${frappe.utils.escape_html(result.provider || "")}</p>
            <p><strong>Profile:</strong> ${frappe.utils.escape_html(result.profile || "")}</p>
            <p><strong>Model:</strong> ${frappe.utils.escape_html(result.model || "")}</p>
            <p><strong>Endpoint:</strong> ${frappe.utils.escape_html(result.endpoint || "")}</p>
            <p><strong>API Key:</strong> ${frappe.utils.escape_html(result.has_api_key ? "Configured" : "Not configured")}</p>
        ...
        const message = error?.message || "AI provider connection test failed.";
        dialog.get_field("result").$wrapper.html(`
          <div>
